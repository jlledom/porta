...
class SecureHeadersTest < ActionDispatch::IntegrationTest
  ...
  test 'adds secure headers in the response' do
    ...
    assert_equal "default-src 'self'; font-src 'self' ; img-src 'self' data: ; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.google.com/recaptcha/ https://www.gstatic.com/recaptcha/ ; style-src 'self' 'unsafe-inline' ; frame-src 'self' https://www.google.com/recaptcha/ https://www.gstatic.com/recaptcha/; connect-src *", response.headers['Content-Security-Policy']
  end
